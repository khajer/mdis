      ...
      if (dataBuffer.includes("\r\n") || dataBuffer.endsWith("\n")) {
        ...
        if (headerParts.length >= 2) {
          ...
          if (method === "set") {
            ...
            // Check if the protocol format is correct (empty line between header and value)
            let valueLine = 2;
            let expireProvided = false;

            if (parts.length > 2 && parts[1] !== "") {
              // Check if this is a duration specification
            ...
            if (parts.length <= valueLine || parts[valueLine - 1] !== "") {
              socket.write(`Err\r\n`);
              return;
            }

            const value = parts[valueLine] || "";
            let responseText = "insert completed";

            if (expireProvided) {
              const durationParts = parts[1].split(" ");
              const exp = durationParts[1];
